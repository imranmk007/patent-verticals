...
#
# Analysis set: the cross-year sample (1,000 apps/year, 2004-2018, 15k total)
# cached at data/hupd_crossyear.parquet by data-load.py. Swap DATA_FILE to
# data/sample-jan-2016.tar.gz-derived data if you want the one-month set.
#
# Outputs:
...
#
# Run: ./.venv/bin/python visualize.py
...
DATA_FILE = BASE / "data" / "hupd_crossyear_25000_clean.parquet"
...
def write_stats(df):
    ...
    print(f"  wrote stats/text_length_stats.csv")

    summary = {
        "dataset": "HUPD cross-year sample (1,000 apps/year, 2004-2018)",
...
def main():
    print(f"Loading {DATA_FILE.relative_to(BASE)} ...")
    df = load()
