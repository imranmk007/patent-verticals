"""
BGE-small-en-v1.5 dense retrieval over lens-db-translated.json.
Indexes title + abstract. Embeddings cached to disk after first run.

To run: set QUERY_FILE below and hit Run.
First run downloads the model (~130MB) and builds embeddings (~2-3 min).
Subsequent runs load from cache and are fast.

Key difference from MiniLM: BGE is tuned for retrieval and expects a short
instruction prefix on the query side only (not on indexed documents).
"""
...
def query_index(model: SentenceTransformer, query_text: str, embeddings: np.ndarray,
    ...
    # BGE uses a query prefix to signal retrieval intent
    prefixed_query = QUERY_PREFIX + query_text
    query_vec = model.encode([prefixed_query], convert_to_numpy=True, normalize_embeddings=True)[0]

    t0 = time.perf_counter()
    # embeddings already normalized at build time, so dot product == cosine similarity
...
def print_results(results: list[dict], query_file: str, index_time: float, query_time: float) -> None:
    print(f"\n{'='*70}")
    print(f"  BGE-small-en-v1.5 Retrieval Results")
...
def save_results(results: list[dict], query_file: str, index_time: float, query_time: float) -> Path:
    ...
    out_path = CACHE_DIR / f"bge_{Path(query_file).stem}.json"
