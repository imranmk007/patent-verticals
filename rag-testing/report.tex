...
\[
  k = \text{clamp}\big(\text{round}(0.1 \times \text{num\_chunks}),\ 2,\ 40\big)
\]

In other words, I average the $k$ best-scoring chunks for each document, where $k$
is 10 percent of the total chunk count. I set a floor of 2 for $k$, so short
documents still get some averaging instead of pure max-pooling. I also set a cap of
40, so very long documents do not dilute back toward a corpus-wide mean, which would
